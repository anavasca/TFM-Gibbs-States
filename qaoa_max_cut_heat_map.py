...
#tempeatures = [0.1, 0.2, 0.3, 0.5, 0.7, 1, 1.3, 1.5, 1.7, 2, 2.3, 2.5, 3, 4, 5, 6, 7, 8, 9, 10, 20, 30, 50]
tempeatures = np.arange(0.6, 30.1, 0.5)
deltas = [0.7, 0.9, 1.5, 1.7, 2, 2.5, 3, 3.5, 4, 7, 10, 20, 30]

len_deltas = len(deltas)

for i in tests:

    G = nx.erdos_renyi_graph(n_nodes, pb_conexion, seed = 3)
    #G = random_uniform_weights(nodes = n_nodes)
    ...
    for temperature in tempeatures:
        ...
        for delta in deltas:
            ...
            random_state = create_random_state(solutions_dict = dict_values, max_energy = energy_hadamard_state)
        ...
        path = f"logger_data/logger_data_qaoa_performance_gibbs_states/logger_data_qaoa_performance_gibbs_states_heat_map/case_{i}_heat_map_n_{n_nodes}_{n_layers}_layers_temperature_{round(temperature, 3)}.json"

        df.to_json(path, orient="records")
    
    """   
    ruta_json = f'logger_data/logger_data_qaoa_performance_gibbs_states/logger_data_qaoa_performance_gibbs_states_heat_map/case_{i}_heat_map_n_8_5_layers_temperature_{tempeatures[0]}.json'

    df_final = pd.read_json(ruta_json)

    for z in tempeatures[1:]: 

        ruta_json = f'logger_data/logger_data_qaoa_performance_gibbs_states/logger_data_qaoa_performance_gibbs_states_heat_map/case_{i}_heat_map_n_8_5_layers_temperature_{z}.json'

        df_l = pd.read_json(ruta_json)
        
        df_final = pd.concat([df_final, df_l], ignore_index=True)
        
    path = f"logger_data/logger_data_qaoa_performance_gibbs_states/logger_data_qaoa_performance_gibbs_states_heat_map/case_{i}_heat_map_n_8_layers_5.json"
